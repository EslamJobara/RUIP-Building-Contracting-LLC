...
const handleServiceClick = () => {
  navigate('/services'); // Removes the service ID
};


  const handleIconClick = (event: React.MouseEvent, serviceId: string) => {
    event.stopPropagation(); // Prevent the card's click event from firing
    navigate('/services'); // Navigate to the main services page
  };

  return (
    <section id="services" className="pt-16 pb-16 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 
          ref={headingRef}
          className="text-3xl md:text-4xl font-bold text-center text-[#104484] mb-12 scroll-reveal"
        >
          Our Services
        </h2>
        ...
        <div 
          ref={servicesRef}
          className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 stagger-children scroll-reveal"
        >
          {services.map((service, index) => (
            <button
              key={index}
              onClick={handleServiceClick}
              className="group bg-white p-8 rounded-lg shadow-md hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 hover:bg-[#104484]/5 text-left focus:outline-none focus:ring-2 focus:ring-[#104484] focus:ring-offset-2"
            >
              <service.icon 
                onClick={(event) => handleIconClick(event, service.id)} // Add onClick handler for the icon
                className="w-12 h-12 text-[#104484] mb-4 transform transition-transform duration-500 group-hover:scale-110 group-hover:rotate-12 cursor-pointer" // Add cursor-pointer
              />
